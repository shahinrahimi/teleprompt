...
func (s *SqliteStore) GetPromptsByUserID(userID int64) ([]models.Prompt, error) {
	rows, err := s.db.Query(models.SELECT_PROMPTS_BY_USER_ID, userID)
	if err != nil {
		return nil, err
	}
	var prompts []models.Prompt
...
func (s *SqliteStore) DeletePromptsByUserID(userID int64) error {
	if _, err := s.db.Exec(models.DELETE_PROMPTS_BY_USER_ID); err != nil {
		s.l.Printf("error deleting prompts for user: %v", err)
		return err
	}
	return nil
}
